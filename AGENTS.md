...
平台包含一个管理后台，用于管理用户、容器生命周期、出口 IP 分配和到期时间。每个容器都预装 `Codex`，并且所有网络流量都必须通过指定出口 IP 的全局隧道路由发送，不能出现 DNS、WebRTC 或其他类型的直接泄漏。
...
- 每次批量生成或修改 `.planning/`、`.cursor/`、`.Codex/` 等工具链文件后，必须检查是否引入了绝对路径或个人信息。
<!-- GSD:conventions-end -->
...
- 用户容器：承载 OpenSSH、Shell 工具和 `Codex`
...
> 本节由 `generate-Codex-profile` 管理，请不要手工修改。
<!-- GSD:profile-end -->
