...
**Current milestone:** v1.3 管理后台凭据与 SSH 密钥增强
...
- [ ] 所有时区选择器都使用完整时区列表，选项统一显示 UTC 偏移（例如 UTC-08:00、UTC+08:00）
- [ ] 出口 IP 的创建/编辑入口改为对话框（Dialog）交互，不再使用右侧抽屉
- [ ] 创建用户时支持手动填写登录密码和 SSH 密码；留空时允许系统自动生成
- [ ] 用户详情页支持管理员修改登录密码和 SSH 密码，可手填或随机生成（提交前必须生成有效值）
- [ ] SSH 密钥模型拆分为登录授权公钥（进入容器）和外联身份密钥对（容器连接外部主机/仓库）
- [ ] 两类 SSH 密钥都支持多条记录，并在变更后实时同步到运行中容器（离线容器在下次启动补齐）
...
- v1.2 在 Phase 11/12 后暂停推进，未完成项转入后续里程碑继续交付。
