...
**Current milestone:** v1.3 claude-shell 本地透明代理
...
- [ ] 单一 Go 二进制即 `claude` 命令，用户下载后直接替换本机 claude，内部透明启动 Docker 容器
- [ ] 容器内通过官方安装脚本安装 Claude Code（Bun standalone），不依赖 npm 或 spoof.js
- [ ] 系统级指纹伪造：entrypoint 预生成 /etc/machine-id、bind mount /proc/cpuinfo 和 /proc/meminfo
- [ ] sing-box tun 全流量代理 + nftables 默认拒绝，外网走代理出口，DNS 不泄漏
- [ ] 本地流量（localhost / 192.168.x / 10.x / 172.16.x）通过 host-gateway 回连宿主机
