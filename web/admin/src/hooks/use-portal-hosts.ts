...
export function useRestartMyHostVNC() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, hostId) => {
      qc.invalidateQueries({ queryKey: ["portal", "hosts", hostId] });
      qc.invalidateQueries({ queryKey: ["portal", "hosts", hostId, "vnc-status"] });
    },
  });
}

export function useMyHostVNCStatus(hostId: string, enabled = true) {
  return useQuery({
    queryKey: ["portal", "hosts", hostId, "vnc-status"],
    queryFn: () => portalApiFetch<VNCServiceStatus>(`/hosts/${hostId}/vnc/status`),
    enabled: !!hostId && enabled,
    refetchInterval: enabled ? 5000 : false,
  });
}

export function useStartMyHostVNC() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (hostId: string) =>
      portalApiFetch<{ status: string }>(`/hosts/${hostId}/vnc/start`, {
        method: "POST",
      }),
    onSuccess: (_data, hostId) => {
      qc.invalidateQueries({ queryKey: ["portal", "hosts", hostId] });
